...
Each rung notifies via the :class:`~airacare_foundry.tools.notify.NotificationTool` and arms an
ack window on a :class:`~airacare_foundry.tools.escalation_timer.Scheduler`. An acknowledgement
resolves the ladder and cancels the pending timer; a timeout advances to the next rung. The
final rung (emergency) is terminal. This is the concrete "long-running autonomous agent" and
lives entirely in T2 — it never touches the synchronous T1 response.
"""
...
from airacare_foundry.contracts import ActionChannel, CloudAction, CloudAssessment, DailyLivingEvent
from airacare_foundry.tools.escalation_timer import Scheduler, ThreadScheduler
