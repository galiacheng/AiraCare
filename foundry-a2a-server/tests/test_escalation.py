"""Escalation ladder tests: ack-tracked family -> community -> emergency, driven deterministically.

Uses :class:`ManualScheduler` so ack windows and advancement are asserted without wall-clock
sleeps. The ladder is off the T1 safety path and only ever runs for L3.
"""

from __future__ import annotations

from airacare_foundry.agents.escalation import (
    DEFAULT_LADDER,
    EscalationAgent,
    LadderStatus,
)
...
def test_ladder_advances_through_all_rungs_without_ack() -> None:
    ...
    # No ack within the family window -> community.
    sched.advance(DEFAULT_LADDER[0].ack_window_seconds)
    assert session.current_channel == "community"
    assert session.status is LadderStatus.RUNNING

    # Still no ack within the community window -> emergency (terminal).
...
def test_ack_resolves_and_stops_escalation() -> None:
    ...
    # A late timeout must not advance a resolved ladder.
    sched.advance(10_000)
    assert [a.channel for a in notifier.sent] == ["family"]
    # A second ack is a no-op.
    assert session.acknowledge() is False
