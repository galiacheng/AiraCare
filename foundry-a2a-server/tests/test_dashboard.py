"""Tests for the live care dashboard — data layer + the stdlib HTTP server.
...
from airacare_foundry.dashboard.data import EVENT_TYPES, LEVELS, DashboardData
from airacare_foundry.dashboard.server import _make_handler, build_data
from airacare_foundry.store.local import LocalEventStore, seeded_local_store
from airacare_foundry.tools.demo_seed import DEFAULT_PATIENT_ID, seed_event_store
...
def test_http_healthz_and_dashboard() -> None:
    config = FoundryConfig(patient=PatientConfig(id=PATIENT, name="Grandpa Zhang"))
