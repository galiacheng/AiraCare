...
:class:`DashboardData` wraps any :class:`~airacare_dashboard.stores.EventStore` (local SQLite or
Cosmos) plus the patient-state store and reuses the Cognitive-Trend and Briefing analytics, so
every number on the dashboard matches what the hosted agent recorded. It never touches the
real-time safety path — it only *reads* the append-only event log.
...
Privacy invariant: only derived :class:`~airacare_dashboard.stores.RecordedEvent` data is read.
"""
...
from airacare_dashboard.analytics import (
    Briefing,
    BriefingAgent,
    CognitiveTrendAgent,
    default_biomarker,
...
from airacare_dashboard.stores import EventStore, PatientStateStore, RecordedEvent
...
class DashboardData:
    """Turns a patient's filed events into the dashboard payload (read-only)."""
    ...
    def events_table(self, records: list[RecordedEvent], *, limit: int = 200) -> list[dict]:
        """Flattened rows for the raw-events table (most recent first)."""
