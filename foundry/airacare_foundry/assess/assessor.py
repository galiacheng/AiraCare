...
the Foundry orchestrator is a true drop-in for the edge's local stub. It is the synchronous
T1 tier: pure rule evaluation that always returns a valid :class:`CloudAssessment` promptly —
off the edge's safety path (the edge's report worker only waits ~5s before store-and-forward).
...
state (or a ``moderate``-stage patient with no unresolved risk) the output is byte-identical
to the edge stub, preserving the drop-in parity guarantee.
"""
...
from airacare_foundry.store.base import DiseaseStage, PatientState
