...
These are copied verbatim from ``edge/airacare_edge/cloud/contracts.py`` so the Foundry
orchestrator and the edge agent stay wire-compatible while remaining independent
deployables. Only :class:`DailyLivingEvent` crosses the privacy boundary from the edge —
it is a *report* of what the edge saw AND already did (the edge decides and acts on its
own; the cloud never gates the action). The cloud returns an async :class:`CloudAssessment`
(with a ``policy_version`` piggyback hint) and, when policy changes, an
:class:`EdgePolicyUpdate` fetched lazily.

Keep this file in sync with the edge contracts — the A2A / JSON-RPC payloads must match
field-for-field.
"""
