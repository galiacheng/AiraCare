"""Data contracts at the cloud boundary — a byte-compatible copy of the edge models.

Copied verbatim from ``edge/airacare_edge/cloud/contracts.py`` (and kept identical to
``foundry-a2a-server/airacare_foundry/contracts.py``) so this Foundry hosted agent stays
wire-compatible with the edge while remaining an independent deployable. Only
:class:`DailyLivingEvent` crosses the privacy boundary from the edge — it is a *report* of what
the edge saw AND already did (the edge decides and acts on its own; the cloud never gates the
action). The cloud returns a considered :class:`CloudAssessment` (with a ``policy_version``
piggyback hint) and, when policy changes, an :class:`EdgePolicyUpdate`.

Keep this file in sync with the edge contracts — the A2A payloads must match field-for-field.
"""
...
class CloudAssessment(BaseModel):
    """Cloud's considered view of a reported event — never gates the edge.
